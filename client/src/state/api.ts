...
export const api = createApi({
  ...
  endpoints: (build) => ({
    getAuthUser: build.query<User,void>({
      queryFn: async (_, _queryApi, _extraoptions, fectchWithBQ) =>{
        try {
          const session = await fetchAuthSession();
          const {idToken} = session.tokens ?? {};
          ...
          let userDetailsResponse  = await fectchWithBQ(endpoint);
    ...
    getCurrentResidences: build.query<Property[], string>({
      ...
      providesTags: (result) =>
        result && Array.isArray(result)
          ? [
              ...result
                .filter((item): item is Property => Boolean(item?.id))
    ...
    getManagerProperties: build.query<Property[], string>({
      ...
      providesTags: (result) => {
                ...
                const validItems = result.filter(
                  (item): item is Property => Boolean(item && item.id)
                );
